...
**Write path (sync + ingest):** Beat fires `sync_all_sources` (default every 5 min, configurable via `SYNC_INTERVAL_MINUTES`) → Celery worker enqueues one `sync_source_task` per ACTIVE source → sync engine (`app.sources.sync.run_sync`) loads `Source` row, decrypts `config_encrypted` (AES-256-GCM with `KG_ENCRYPTION_KEY`), builds the connector via `build_connector(source)`, validates credentials (mark source `auth_failed` on auth error), calls `list_changes(cursor)` to discover new/updated/deleted docs → for each doc: skip if `size_bytes > MAX_DOC_SIZE_MB` (default 50 MB), else `fetch_doc` → upload raw bytes to MinIO under `{type}/{source_id}/{doc_id}` → upsert `Document` row by `(source, source_id)` → enqueue `ingest_doc_task(doc_id)` (best-effort; broker outage leaves the doc in `discovered`, next sync re-enqueues) → publish progress event to Redis channel `kg:sync:{job_id}:progress` → mark `SyncJob` `COMPLETED` / `PARTIAL` / `FAILED` → persist next cursor. Drive push notifications land on `POST /api/v1/webhooks/google-drive`; the handler verifies `X-Goog-Channel-Token`, ignores the initial `sync` ping, and enqueues a `sync_source_task(triggered_by="webhook")` for `update` / `exists` states. The SSE endpoint `GET /api/v1/sync-jobs/{id}/stream` (planned) subscribes to the same pub/sub channel and streams JSON frames to the admin dashboard.

**Ingest path:** `ingest_doc_task` (in `app.tasks.ingest`) loads the `Document` row, downloads the raw bytes from MinIO via `download_doc`, parses with Unstructured (`app.pipeline.parser.parse_bytes`), chunks with the heading-aware + recursive splitter (`app.pipeline.chunker.chunk_by_sections`, default 512 target / 1024 max tokens, 10% overlap), detects language per chunk (vi / en / zh / und via `langdetect`), embeds with bge-m3 in batches of `EMBEDDING_BATCH_SIZE` (default 8 on CPU, vectors L2-normalized), bulk-upserts `PointStruct` records to the `chunks` Qdrant collection in 500-point batches with payload `{doc_id, group_ids, language, status, chunk_index, section_title, indexed_at, source, source_id}`, upserts the corresponding `Chunk` rows in PG (with `qdrant_point_id`, `embedding_model='bge-m3-v1.0.0'`, `embedding_dim=1024`), and marks the `Document` row `active` with `indexed_at` set. The worker pre-warms the bge-m3 model on the `worker_init` Celery signal so the first task after boot does not pay the 5+ second load. Empty documents (image-only PDFs) and parse errors mark the document `failed` with a short `error_message`; the caller never persists orphan chunks because the chunk insert is the last step before the status update.

**Read path (query):** User → API auth (JWT) → semantic cache check (`kg:query:result:{query_hash}:{filter_hash}`, 24h TTL) → cache hit returns the prior answer + citations; otherwise: detect query language (vi/en/zh) → embed query via bge-m3 (Redis-cached 5 min) → load user groups for the permission filter (`user.groups ∩ doc.access_groups`) → hybrid search: Qdrant cosine (`group_ids ∈ user.groups` + `status=active`, top-20) in parallel with PostgreSQL FTS on `chunks.tsv` (`to_tsquery('simple', plainto_tsquery(text))` joined with `document_groups`, top-20), merged with Reciprocal Rank Fusion (k=60, top-20) and hydrated with `chunk_text` from PG → rerank top candidates via bge-reranker-v2-m3 cross-encoder (top-5) → resolve doc titles + presigned URLs (one batched PG + MinIO call) → LLM call via LiteLLM proxy (gpt-4o-mini default, ollama/llama3 fallback routed through a process-local `CircuitBreaker` with 5-failure threshold + 60s cool-down) → build prompt from `KG_PROMPT_VERSION = "1.0.0"` (answer only from sources, numbered `[N]` citations, surface conflicts, no translation, say "no information" when sources empty) → extract `[N]` tokens from the response, map to `Citation` objects (title, section, page, source provider, last_updated, presigned URL, 200-char snippet) → persist `Query` row (status, cost, latency, retrieved_chunks JSONB, answer, warnings, llm_model) → audit log (best-effort, fire-and-forget) → set the semantic cache. No-result branches: empty index → localized "could not find" (E5) + popular-doc suggestions; user in zero groups → localized "N results, no permission" (E9) with the count (not the titles — leak defense). Rate limit: per-user sliding window in Redis (default 30 queries/min).
